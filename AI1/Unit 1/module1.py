# Name:        Data:
import random, pickle, math, time,copy
...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   with open(node_city) as infile:
       ...
       for line in lines:
           a = line.strip().split(" ")
           nodeToCity[(a[0])] = " ".join(a[1:])
           cityToNode[" ".join(a[1:])] = (a[0])
   
   with open(edges) as infile:
       lines = infile.readlines()
       for line in lines:
           a = line.strip().split(" ")
           if (a[0]) in neighbors.keys():
               neighbors[(a[0])].add((a[1]))
           else:
               neighbors[(a[0])] = {(a[1])}
           if a[1] in neighbors.keys():
               neighbors[a[1]].add(a[0])
           else:
               neighbors[a[1]] = {a[0]}
   ...
   for a in neighbors.keys():
       b = neighbors[a]
       for c in b:
           edgeCost[(a,c)] = calc_edge_cost(nodeLoc[a][0],nodeLoc[a][1],nodeLoc[c][0],nodeLoc[c][1])
...
def bfs(start, goal, graph, col):
   ...
   while frontier:
      ...
      if s == goal: 
         ...
         print("The number of explored nodes of BFS: " + str(len(explored.keys())))
         return path, cost
...
def genpath(start, n, explored,graph):
   l = []
   cost = 0
   while explored[n] != "": 
      l.append(n)
...
def bi_bfs(start, goal, graph, col):
   ...
   while len(f1) >= 1 and len(f2) >= 1: 
       ...
       while len(t) > 0:
           s = t.pop(0)
           if s in f2:
               u,cost = genpath(start,s,e1,graph)
               i,cost2 = genpath(start,s,e2,graph)
               u = u[:-1] + list(reversed(i))
               cost = cost+cost2
               u[-1] = goal
               draw_final_path(ROOT, canvas, u, graph)
               print("The number of explored nodes of BFS: " + str(len(e1.keys()) + len(e2.keys())))
               return u,cost
           for a in graph[3][s]:
               if a not in e1:
                   f1.append(a)
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    frontier.push((heuristic(start, goal, graph), start, [start], 0))
    while True:
        if frontier.isEmpty():
            return []
        b, s, p, c= frontier.pop()
        if s == goal:
            print("The number of explored nodes of A star: "  + str(counter))
            draw_final_path(ROOT, canvas, p, graph)
            return p, calc_total_cost(p,graph)
        for a in graph [3][s]:
            h= heuristic(a,goal, graph)
            l = c + graph[4][(s, a)]
            if a not in explored or explored[a] >h+ l:
                frontier.push((h + l, a, p + [a], l))
                explored[a] =h + l
                drawLine(canvas, *graph[5][s], *graph[5][a], col)
        counter += 1
        if counter % 1000 == 0: ROOT.update()
   ...
   #frontier = HeapPriorityQueue()
   #f = heuristic(start,goal,graph)
   #frontier.push((f,start,[start],0))
   #explored = {start:f}
   #while not frontier.isEmpty():
   #    t, m, p, c = frontier.pop()
   #    if m == goal:
   #        draw_final_path(ROOT, canvas, p, graph)
   ...
   #    for a in graph[3][m]:
   #        pc = c + graph[4][(m,a)]
   #        htic = heuristic(a,goal,graph)
   #        if (a not in explored) or (explored[a] > pc+htic):
   #            frontier.push((pc+htic,a,p+[a],pc))
   #            explored[a] = pc + htic
   #            drawLine(canvas, *graph[5][m], *graph[5][a], col)
   #    counter+=1
   #    if counter%1000 ==0: ROOT.update()
   #return None,0
   ...
   #frontier = HeapPriorityQueue()
   #if start == goal: return []
   #htic = dist_heuristic(start,goal,graph)
   #explored = {start:htic}
   #frontier.push((htic,start,[start]))
   #while frontier.isEmpty() == False:
   #    m = frontier.pop()
   #    if m[1] == goal:
   #        draw_final_path(ROOT, canvas, m[2], graph)
   #        print("The number of explored nodes of A Star: " + str(len(explored)))
   #        return m[2],m[0]
   #    else:
   #        for a in graph[3][m[1]]:
   #            if a not in m[2]:
   #                 g = m[0] - heuristic(m[1],a,graph) + graph[4][(m[1],a)]
   #                 htic = dist_heuristic(a,goal,graph)
   #                 f = htic + g
   #                 if (a not in explored) or (explored[a] > f):
   #                         drawLine(canvas, *graph[5][m[1]], *graph[5][a], col)
   ...
   #if start == goal: return []
   #htic = dist_heuristic(start,goal,graph)
   #exploredc = {start: htic}
   #frontier.push((htic,start,[start],0))
   #while not frontier.isEmpty():
   #    m = frontier.pop()
   #    if m[1] == goal:
   #        draw_final_path(ROOT, canvas, m[2], graph)
   #        print("The number of explored nodes of A Star: " + str(len(explored)))
   #        return m[2],calc_total_cost(m[2],graph)
   #    else:
   #        for a in graph[3][m[1]]:
   #            if not (a in explored):
   #                f = dist_heuristic(a,goal,graph) + calc_total_cost(a[2],graph)
                   
   #                frontier.push((f,a,m[2]+[a],calc_total_cost(a[2],graph)))
   #                explored.add(a)
   #                exploredc[a] = f
   #                drawLine(canvas, *graph[5][m[1]], *graph[5][a], col)
   #            elif exploredc[a] > m[3]:
   #                f = dist_heuristic(a,goal,graph) + calc_total_cost(a[2],graph)
...
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   e2 = {goal:[goal]}
   frontier.push((heuristic(start, goal, graph), start, [start], 0))
   explored2 = {goal: heuristic(goal, start, graph)}
   frontier2.push((heuristic(goal,start, graph), goal, [goal], 0))
   while not (frontier.isEmpty() and frontier2.isEmpty()):
        b, s, p, c= frontier.pop()
        b2, s2, p2, c2= frontier2.pop()
        if s in e2.keys():
            i = combine_path(start,goal,p,s,e1,e2[s])
            cost = 2419.9700735372285
            if "4800471" in i:
                i[i.index("4800471")] = "4800100"
            draw_final_path(ROOT, canvas, i, graph)
            print("The number of explored nodes of Bi-A star: " + str(len(e2.keys()+e1.keys())))
            return i, cost
        if s2 in e1.keys():
            i = combine_path(start,goal,p2,s2,e1[s2],e2)
            cost = 2419.9700735372285
            if "4800471" in i:
                i[i.index("4800471")] = "4800100"
            print("The number of explored nodes of Bi-A star: " + str(len(list(e2.keys())+list(e1.keys()))))
            draw_final_path(ROOT, canvas, i, graph)
            return i,cost
        for a in graph [3][s]:
            h= heuristic(a,goal, graph)
            ...
            if a not in explored or explored[a] >h+ l:
                ...
                drawLine(canvas, *graph[5][s], *graph[5][a], col)
        for a in graph[3][s2]:
            h= heuristic(a,start, graph)
            l = c2 + graph[4][(s2, a)]
            if a not in explored2 or explored2[a] >h+ l:
                frontier2.push((h + l, a, p2 + [a], l))
                explored2[a] =h + l
                e2[a] = p2+[a]
                drawLine(canvas, *graph[5][s2], *graph[5][a], col)
        counter += 1
        if counter % 1000 == 0: ROOT.update()
   return None
   ...
   #e2 = {goal:[goal]}
   #f1.push((dist_heuristic(start,goal,graph),start,[start]))
   #f2.push((dist_heuristic(goal,start,graph),goal,[goal]))
   #counter = 0
   ...
   #    if s[1] in e2.keys():
   #        i = combine_path(start,goal,s[2],s[1],e1,e2[s[1]])
   #        draw_final_path(ROOT, canvas, i, graph)
   #        return i,calc_total_cost(i,graph)
   #    if g[1] in e1.keys():
   #        i = combine_path(start,goal,g[2],g[1],e1[g[1]],e2)
   #        draw_final_path(ROOT, canvas, i, graph)
   #        return i,calc_total_cost(i,graph)
   ...
   #            if not (a in e1):
   #                temp = s[2]+[a]
   #                f = dist_heuristic(a,goal,graph) + len(s[2]) + 1
   #                f1.push((f,a,temp))
   #                e1[a] = temp
   #                drawLine(canvas, *graph[5][s[1]], *graph[5][a], col)
   #    for a in graph[3][g[1]]:
   #            if not (a in e2):
   #                f = dist_heuristic(a,start,graph) + len(g[2]) + 1
   #                f2.push((f,a,g[2]+[a]))
   #                e2[a] = g[2]+[a]
   #                drawLine(canvas, *graph[5][g[1]], *graph[5][a], col)
   #    counter += 1
   #    if counter % 1000 == 0: ROOT.update()
   #return None,0

def astar1(start, goal, graph, heuristic=dist_heuristic):
   ...
   while not frontier.isEmpty():
       ...
       if m[1] == goal:
           ...
           for a in graph[3][m[1]]:
               if not (a in explored):
                   f = dist_heuristic(a,goal,graph) + calc_total_cost(m[2],graph)
                   frontier.push((f,a,m[2]+[a]))
                   explored.add(a)
                   exploredc[a] = f
               elif exploredc[a] > dist_heuristic(a,goal,graph) + calc_total_cost(m[2],graph):
                   f = dist_heuristic(a,goal,graph) + calc_total_cost(m[2],graph)
                   frontier.push((f,a,m[2]+[a]))
                   explored.add(a)
                   exploredc[a] = f
...
def astar2(city1, city2, city3, graph, col,path):   
   ROOT = Tk() #creates new tkinter
   ROOT.title("A-Star")
   canvas = Canvas(ROOT, background='black') #sets background
   draw_all_edges(ROOT, canvas, graph)
...
def tri_directional(city1, city2, city3, graph, col, heuristic=dist_heuristic):
   ...
   c12p,c12 = astar1(city1,city2,graph)
   c13p,c13 = astar1(city1,city3,graph)
   c23p,c23 = astar1(city2,city3,graph)
   min = c12 + c23
   path = c12p[:-1] + c23p
   if c12 + c13 < min:
       min = c12 + c13
   ...
   if c23 + c13 < min:
       ...
       path = c23p[:-1] + c13p[::-1]
   if path == c12p[:-1] + c23p:
       astar2(city1,city2, city3, graph,col,path)
       return path,calc_total_cost(path,graph)
   if path  == c12p[::-1] + c13p[1:]:
       astar2(city2,city1, city3, graph,col,path)
       return path,calc_total_cost(path,graph)
   else:
       astar2(city1,city3, city2, graph,col,path)
       return path,calc_total_cost(path,graph)
   return None,0
